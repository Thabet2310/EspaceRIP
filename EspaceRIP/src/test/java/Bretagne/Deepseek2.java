    ...
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        for (Row row : excelSheet) {
            ...
            if (statut.equals("Lecture") || statut.equals("Collaboration")) {
                ...
                // Vérifier si la sous-rubrique suivante appartient à la même rubrique et catégorie
                Row nextRow = excelSheet.getRow(row.getRowNum() + 1);
                if (nextRow != null) {
                    String nextCategorie = nextRow.getCell(0).getStringCellValue();
                    String nextRubrique = nextRow.getCell(1).getStringCellValue();

                    if (!nextCategorie.equals(categorie) || !nextRubrique.equals(rubrique)) {
                        // Si la catégorie ou la rubrique change, on sort de la boucle
                        break;
                    }
                }
            }
