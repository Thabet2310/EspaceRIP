...
public class GERS_OSA1 {
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        String NomProfilUtilisateur = "GERS_OSA1";
        ...
        String C1 = "Mes esssentiels"; // Categorie
        String[] R1C1 = {"hgh"}; // Rubriques de catégorie 1
        String[][] SR_R1C1 = {{"hhgh"}};

        // Catégorie : Mon réseau
        String C2 = "ghghghghg"; // Categorie
        String[] R2C2 = {"ghh"}; // Rubriques de catégorie 2
        String[][] SR_R2C2 = {
        ...
        String[] R3C3 = {"rubriqueerroné", "Mes factures"}; // Rubriques de catégorie 3
        String[][] SR_R3C3 = {
                // Sous-rubriques de rubrique 1 de catégorie 3
                {"Mes éléments"},{"Mes éléments de facturation usagers"}};
        ...
        for (int i = 0; i < 3; i++) {
            ...
            // Si la catégorie n'existe pas, passer à la suivante
            if (!elementExiste(driver, By.xpath(pathMenuTitle))) {
                addDataToExcel(sheet, rowNum++, categorie, "", "", "", "");
                System.out.println("La catégorie '" + categorie + "' n'existe pas. Passage à la catégorie suivante.");
                continue;
            }
            ...
            for (int j = 0; j < rubriques.length; j++) {
                ...
                if (!elementExiste(driver, By.linkText(rubrique))) {
                    System.out.println("La rubrique '" + rubrique + "' n'existe pas. Passage à la rubrique suivante.");
                    // Stocker "RIEN" pour toutes les sous-rubriques correspondantes
                    for (String sousRubrique : sousRubriqueArray) {
                        addDataToExcel(sheet, rowNum++, "", rubrique, sousRubrique, "RIEN", "RIEN");
                    }
                    continue;
