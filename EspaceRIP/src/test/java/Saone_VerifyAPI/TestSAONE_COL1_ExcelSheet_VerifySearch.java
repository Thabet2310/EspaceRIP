...
public class TestSAONE_COL1_ExcelSheet_VerifySearch {
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        for (String element : SR_R1C1) {
            ...
            boolean champRechercheCliquable = false;

            if (elementExiste) {
                // Vérifier si le champ de recherche est cliquable
                try {
                    WebElement champRecherche = driver.findElement(By.id("inplaceSearchDiv_WPQ2_lsinput"));
                    Thread.sleep(2000); // Ajouter une pause avant de cliquer
                    champRecherche.click();
                    Thread.sleep(2000); // Ajouter une pause après le clic
                    champRechercheCliquable = true;
                } catch (Exception e) {
                    champRechercheCliquable = false;
                }
            }

            String statutElement = elementExiste ? (champRechercheCliquable ? "OK (API OK)" : "OK (API KO)") : "KO";

            addDataToExcel(sheet, rowNum++, "", "", element, statutElement);

            if (elementExiste) {
                driver.findElement(By.linkText(element)).click();
                Thread.sleep(10000); // Ajouter une pause après le clic sur le lien

                if (champRechercheCliquable) {
                    System.out.println("\033[32m" + element + " OK (API OK)\033[0m");
                } else {
                    System.out.println("\033[31m" + element + " OK (API KO)\033[0m");
                }

                driver.navigate().back();
                Thread.sleep(2000); // Ajouter une pause après le retour en arrière
            } else {
...
/*
        String R2C1 = "Mes offres & contrats usagers";
        String[] SR_R2C1 = {
        ...
        String R2C2 = "Ma vie de réseau";
        String[] SR_R2C2 = {"Mes signalisations réseaux"};
        ...
        String[] SR_R1C3 = {"Mes redevances et contributions"};
        ...
        String[] SR_R2C3 = {"Mes comités techniques"};
...
*/
        try (FileOutputStream fileOut = new FileOutputStream("rapport_recette_SAONE_COL.xlsx")) {
